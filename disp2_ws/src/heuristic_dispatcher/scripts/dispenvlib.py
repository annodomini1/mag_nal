...
class TaskBuffers(object):
    ...
    def sort_by_deadline(self):
        """ Tasks within individual robot's buffer are sorted by descending
        deadline.
        """
        ...
        for i in range(self.no_robots):
            if isinstance(self.all_buffers[i], np.ndarray):
                ...
                sorted_indi_buff = individual_buffer[individual_buffer[:, -1].argsort()[::-1]]
                self.all_buffers[i] = sorted_indi_buff
            else:
                pass
...
class DispatcherTools(object):
    ...
    def get_rectilinear_distances(self, goal, poses):
        
        def euclidean_norm((point_x1, point_y1), (point_x2, point_y2)):
            ed = np.sqrt((point_x1 - point_x2) ** 2 + (point_y1 - point_y2) ** 2)
            return ed
        ...
        for i in range(len(x_pts)):
            dist = euclidean_norm((goal[0], goal[1]), (x_pts[i], y_pts[i]))
